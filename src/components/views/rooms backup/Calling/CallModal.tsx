...
const CallModal: React.FC = () => {
  ...
  const incomingCallUrl = "https://firebasestorage.googleapis.com/v0/b/laaleh-2451e.appspot.com/o/ring.ogg?alt=media&token=f7ff8103-a1e5-42a6-a7d3-12c73b5bda2e";
  const outgoingCallUrl = "https://firebasestorage.googleapis.com/v0/b/laaleh-2451e.appspot.com/o/ringback.ogg?alt=media&token=308068d4-a9af-4b1f-a4da-d8cc1f237685";
  ...
  const handleEndCall = async () => {
    if (roomId) {
      ...
      if (isIncoming && !isOngoing) {
        ...
        const currentUserId = Object.keys(participants).find(id => participants[id].username === callState.callerInfo?.username) || '';
                ...
                <CustomTooltip title="Participants in call">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500">
                      {Object.values(remoteStreams).filter(stream => stream !== null).length + 1}
                    </span>
                  </div>
                </CustomTooltip>
              </div>
          ...
          {isVideo ? (
            <div>
              <div className={`video-grid ${
                  ...
                  <div className="video-controls">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        ...
                        <CustomTooltip title={isVideoEnabled ? 'Turn off camera' : 'Turn on camera'}>
                          <button
                            className={`p-2 rounded-lg transition-all ${
                              isVideoEnabled
                                ? 'bg-white/20 hover:bg-white/30'
                ...
                {/* Remote Videos */}
                {Object.entries(remoteStreams).filter(([_, stream]) => stream !== null).map(([userId, stream]) => {
