...
export class StopGapWidgetDriver extends WidgetDriver {
     ...
     * @experimental Part of MSC4140 & MSC4157
     * 
     * Updates a delayed event. This is used by Element Call widgets for session management.
     * 
     * Known behavior: Errors may appear in console logs during widget cleanup when delayed
     * events fail to update (typically "Failed to override function" errors). These errors
     * are expected and handled gracefully - they occur when widget messaging is cleaned up
     * before delayed events complete. The errors are suppressed here to prevent the
     * MembershipManager from shutting down, which would break call functionality.
     */
    public async updateDelayedEvent(delayId: string, action: UpdateDelayedEventAction): Promise<void> {
        ...
        try {
            ...
            // By catching all errors here, we prevent the shutdown and allow graceful degradation
            // 
            // All delayed event update errors are considered non-fatal because:
            // - They can occur when widgets/sessions are cleaned up before delayed events complete
            // - The server will handle expiry naturally
            // - The delayed event mechanism is experimental (MSC4140/MSC4157) and can fail in various ways
            // - The "Failed to override function" error indicates the widget messaging was destroyed
            //   while a delayed event update was in flight - this is normal during cleanup
            // 
            // Note: These errors may still appear in console logs from the Matrix SDK before
            // being caught here. This is expected behavior and does not indicate a problem.
            logger.debug(
                `Delayed event update failed (this is expected during cleanup): ${error}. ` +
                `The delayed event will be handled by the server.`
            );
            return;
        }
    }
