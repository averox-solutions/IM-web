...
export class SendMessageComposer extends React.Component<ISendMessageComposerProps> {
    ...
    private async checkAndUpdateTagsAfterMessage(roomId: string): Promise<void> {
        ...
        try {
            ...
            logger.log(
                `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Current user RoomList tags for ${roomId}: ${JSON.stringify(
                    myRoomListTags,
            ...
            logger.log(
                `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Other user tag status:`,
                otherUserTagStatus,
            );

            const otherUserHasLeaveTag = !!otherUserTagStatus?.tags?.["m.leave-1-1-chat"];
            logger.log(
                `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Other user has leave tag: ${otherUserHasLeaveTag}`,
            );

            // If neither side has the leave tag, nothing to do
            if (!currentUserHasLeaveTag && !otherUserHasLeaveTag) {
                logger.log(
                    `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Neither current user nor other user has leave tag for room ${roomId}, skipping`,
                );
                return;
            }
            ...
            if (currentUserHasLeaveTag) {
                try {
                    logger.log(
                        `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Current user has leave tag, removing it for user ${myUserId} in room ${roomId}`,
                    );
                    ...
                    logger.log(
                        `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Removed leave tag for current user and updated RoomList for room ${roomId}`,
                    );
                } catch (e) {
                    logger.error(
                        `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Failed to remove leave tag for current user via bulkUpdateRoomTags in room ${roomId}:`,
                        e,
                    );
                }
            }
            ...
            if (otherUserHasLeaveTag && otherUserId) {
                try {
                    logger.log(
                        `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Removing leave tag for other user ${otherUserId} in room ${roomId} via bulkUpdateRoomTags`,
                    );
                    ...
                    logger.log(
                        `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Successfully requested leave tag removal for other user ${otherUserId} in room ${roomId}`,
                    );
                } catch (e) {
                    logger.error(
                        `[SendMessageComposer.checkAndUpdateTagsAfterMessage] Failed to remove leave tag for other user ${otherUserId} via bulkUpdateRoomTags in room ${roomId}:`,
                        e,
                    );
                }
            }
