...
export default class CreateRoomDialog extends React.Component<IProps, IState> {
    ...
    public constructor(props: IProps) {
        ...
        const isVideoRoom = props.type === RoomType.ElementVideo || props.type === RoomType.UnstableCall;

        let joinRule = JoinRule.Invite;
        if (isVideoRoom || this.props.defaultPublic) {
            joinRule = JoinRule.Public;
        } else if (this.supportsRestricted) {
            joinRule = JoinRule.Restricted;
        }
    ...
    private onKeyDown = (event: KeyboardEvent): void => {
        const action = getKeyBindingsManager().getAccessibilityAction(event);
        if (action === KeyBindingAction.Enter) {
            this.onOk();
            event.preventDefault();
            event.stopPropagation();
        }
    ...
    public render(): React.ReactNode {
        ...
        if (!isVideoRoom) {
            if (this.state.joinRule === JoinRule.Restricted) {
                publicPrivateLabel = (
                    <p>
                        {_t("create_room|join_rule_restricted_label", {}, {
                            SpaceName: () => <strong>{this.props.parentSpace?.name ?? _t("common|unnamed_space")}</strong>,
                        })}&nbsp;
                        {_t("create_room|join_rule_change_notice")}
                    </p>
                );
                ...
                publicPrivateLabel = (
                    <p>
                        {_t("create_room|join_rule_public_parent_space_label", {}, {
                            SpaceName: () => <strong>{this.props.parentSpace?.name ?? _t("common|unnamed_space")}</strong>,
                        })}&nbsp;
                        {_t("create_room|join_rule_change_notice")}
                    </p>
        ...
        let visibilitySection: JSX.Element | undefined;
        if (!isVideoRoom && this.state.joinRule === JoinRule.Knock) {
            visibilitySection = (
                <LabelledCheckbox
                    className="mx_CreateRoomDialog_labelledCheckbox"
                    label={_t("room_settings|security|publish_room")}
            ...
            e2eeSection = (
                <>
                    <LabelledToggleSwitch
                        ...
                        onChange={this.onEncryptedChange}
                        value={this.state.isEncrypted}
                        className="mx_CreateRoomDialog_e2eSwitch"
                        disabled={!this.state.canChangeEncryption}
                    />
                    <p>{microcopy}</p>
                </>
        ...
        if (isVideoRoom) {
            ...
            title = this.state.joinRule === JoinRule.Public
                ? _t("create_room|title_public_room")
                : _t("create_room|title_private_room");
        }
        ...
        return (
            ...
            >
                    ...
                    <div className="mx_Dialog_content">
                        ...
                        {!isVideoRoom && (
                            <JoinRuleDropdown
                                label={_t("create_room|room_visibility_label")}
                                labelInvite={_t("create_room|join_rule_invite")}
                                labelKnock={this.askToJoinEnabled ? _t("room_settings|security|join_rule_knock") : undefined}
                                labelPublic={_t("common|public_room")}
                                labelRestricted={this.supportsRestricted ? _t("create_room|join_rule_restricted") : undefined}
                                value={this.state.joinRule}
                                onChange={this.onJoinRuleChange}
                            />
                        )}
                ...
                <DialogButtons
                    primaryButton={isVideoRoom ? _t("create_room|action_create_video_room") : _t("create_room|action_create_room")}
                    onPrimaryButtonClick={this.onOk}
                    onCancel={this.onCancel}
                />
            </BaseDialog>
