        ...
        816: (e, t, n) => {
          ...
          const o = new (class extends i {
            constructor() {
              super();
              try {
                // Try to use window.localStorage for persistence.
                this._storage = window.localStorage;
                this._localStorageDisabled = false;
              } catch (e) {
                console.warn("Local storage is disabled, falling back to custom storage.");
                this._storage = new s(); // Fallback storage if localStorage is not available.
                this._localStorageDisabled = true;
              }
          
              // List of keys to persist even on logout (e.g., saved passphrase, user preferences)
              this._persistentKeys = ["savedPassPhrase", "userPreferences", "otherKeyToKeep"];
            }
            ...
            // Clear method now preserves the persistent keys defined in _persistentKeys
            clear() {
              if (this._localStorageDisabled) {
                // For fallback storage, you need to implement specific logic to preserve persistent data.
                const preservedData = this._preservePersistentData();
                this._storage.clear();
                this._restorePersistentData(preservedData);
              } else {
                // For localStorage, preserve items in _persistentKeys
            ...
            // Function to preserve persistent data in fallback storage
            _preservePersistentData() {
              const preservedData = {};
              this._persistentKeys.forEach((key) => {
            ...
            // Restore persistent data after clearing the fallback storage
            _restorePersistentData(data) {
            ...
            removeItem(key) {
              // Only remove the item if it's not in the persistent keys list
              if (!this._persistentKeys.includes(key)) {
                this._storage.removeItem(key);
                this.emit("changed");
              }
            ...
            serialize(excludeKeys = []) {
              if (this.isLocalStorageDisabled()) {
                return this._storage.serialize(excludeKeys);
              }
              
              const totalItems = this._storage.length;
              const serializedData = {};
              for (let i = 0; i < totalItems; i++) {
                const key = this._storage.key(i);
                if (!excludeKeys.includes(key)) {
                  serializedData[key] = this._storage.getItem(key);
                }
              }
              ...
              s.forEach((e) => {
                ...
                try {
                  ...
                  return void (function (e, t = "") {
                    console.error(t, e),
                      window.onerror &&
                        window.onerror(t, void 0, void 0, void 0, e);
          ...
          class S extends r() {
                ...
                (this._url = (function (e) {
                  return p({
                    ...(arguments.length > 1 && void 0 !== arguments[1]
                      ? arguments[1]
                      : {}),
                    url: `https://${e}/#jitsi_meet_external_api_id=${C}`,
                  });
                })(e, {
                  configOverwrite: l,
                  interfaceConfigOverwrite: d,
            ...
            _createIFrame(e, t, n) {
              const i = `jitsiConferenceFrame${C}`;
              ...
              this._transport.on("event", (e) => {
                ...
                switch (t) {
                  ...
                  case "participant-left":
                    R(this, -1), delete this._participants[i];
                    break;
