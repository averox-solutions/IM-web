...
// Import socket and utility functions for call ended event handling
import { getSocket } from "../Calling/socketInitializer";
...
const emitCallEndedEvent = (roomId: string, toUserIds: string[]): void => {
    try {
        console.log("came in emitCallEndedEvent");
        const socket = getSocket();
        const fromUserId = getCurrentMatrixUserId();

        if (!socket || !fromUserId) {
            console.warn("Cannot emit call ended event: socket or user ID not available");
            return;
        }
        ...
        console.log("📞 Emitting CALL_ENDED event to backend:", callEndedData);
...
const AudioCallInterface: React.FC<{ isVideo: boolean }> = ({ isVideo }) => {
    ...
    // Handle speaker toggle (simplified - just visual feedback)
    const handleSpeakerToggle = (): void => {
        setIsSpeakerOn(!isSpeakerOn);
        console.log(`🔊 Speaker ${!isSpeakerOn ? "enabled" : "disabled"}`);
    };
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <button
                                className={`audio-control-button group-call speaker ${isSpeakerOn ? "active" : ""}`}
        ...
        >
            ...
            >
                ...
                <div className="audio-call-controls">
                    ...
                    <button
                        className={`audio-control-button speaker ${isSpeakerOn ? "active" : ""}`}
                        onClick={handleSpeakerToggle}
                        title={isSpeakerOn ? "Turn off speaker" : "Turn on speaker"}
                    >
                        {isSpeakerOn ? "🔊" : "🔈"}
                    ...
                    .catch((err: any) => {
                        ...
                        if (isVideo) {
                            room.localParticipant
                                ...
                                .then(() => console.log("✅ Camera enabled (fallback)"))
                                .catch((e: any) => console.warn("❌ Camera enable failed (fallback):", e));
                        }

                        room.localParticipant
                            .setMicrophoneEnabled(true)
                            .then(() => console.log("✅ Microphone enabled (fallback)"))
                            .catch((e: any) => console.warn("❌ Microphone enable failed (fallback):", e));
                    });
            });
        }
    }, [room, isVideo]);
...
export const VideoRoom = ({
    ...
    useEffect(() => {
        const handleCallEnded = (data: any) => {
            console.log("📞 Received call_ended event from backend:", data);
            ...
            if (data.roomId === roomId) {
                console.log("📞 Call ended for current room, closing modal");
                onLeave?.();
            }
        };
        ...
        try {
            ...
            if (socket) {
                socket.on("call_ended", (data: any) => {
                    console.log("📞 Received call_ended socket event:", data);
                    // Dispatch custom event for consistent handling
                    const callEndedEvent = new CustomEvent("liveKitCallEnded", {
                        detail: data,
                    });
                    window.dispatchEvent(callEndedEvent);
                });
                console.log("✅ Set up call_ended socket listener");
            }
        } catch (error) {
            console.warn("Failed to set up call_ended socket listener:", error);
        }
        ...
        return () => {
            ...
            try {
                ...
                console.warn("Failed to clean up call_ended socket listener:", error);
            }
        };
    }, [roomId, onLeave]);
