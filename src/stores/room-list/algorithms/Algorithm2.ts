...
export class Algorithm extends EventEmitter {
    ...
    public getOrderedRooms(): ITagMap {
        ...
        for (const [tag, rooms] of Object.entries(roomsMap)) {
            filteredMap[tag] = rooms.filter(room => {
                ...
                // Exclude rooms where user has left, been banned, or been kicked
                const myMembership = room.getMyMembership();
                if (myMembership === KnownMembership.Leave || 
                    myMembership === KnownMembership.Ban || 
                    myMembership === KnownMembership.Kick) {
                    return false;
                }
        ...
        for (const room of memberships[EffectiveMembership.Join]) {
            ...
            if (!inTag) {
                if (DMRoomMap.shared().getUserIdForRoomId(room.roomId)) {
                    ...
                    // Room is not explicitly marked as DM in m.direct account data
                    // Do NOT automatically mark 2-member rooms as DMs - this causes group chats to be misclassified
                    newTags[DefaultTagID.Untagged].push(room);
                }
            }
        }
    ...
    private getTagsOfJoinedRoom(room: Room): TagID[] {
        ...
        if (tags.length === 0) {
            ...
            // Note: We do NOT automatically mark 2-member rooms as DMs.
            // If the room is not in m.direct, it should remain in the default tag.
        }
        if (room.isCallRoom() && (room.getJoinRule() === JoinRule.Public || room.getJoinRule() === JoinRule.Knock)) {
            tags.push(DefaultTagID.Conference);
        }
    ...
    public handleRoomUpdate(room: Room, cause: RoomUpdateCause): boolean {
        ...
        // Handle RoomRemoved specially - if the room doesn't have tags, search all tags and remove it
        if (cause === RoomUpdateCause.RoomRemoved && !this.roomIdsToTags[room.roomId]) {
            // Room doesn't have tags but we need to remove it - search all tags for this room
            let changed = false;
            for (const tagId in this.algorithms) {
                // noinspection JSUnfilteredForInLoop
                const algorithm: OrderingAlgorithm = this.algorithms[tagId];
                if (!algorithm) continue;
                
                // Check if room is in this tag's ordered rooms
                const roomsInTag = algorithm.orderedRooms || [];
                if (roomsInTag.some((r) => r.roomId === room.roomId)) {
                    algorithm.handleRoomUpdate(room, RoomUpdateCause.RoomRemoved);
                    this._cachedRooms[tagId] = algorithm.orderedRooms;
                    this.recalculateStickyRoom(tagId);
                    this.recalculateActiveCallRooms(tagId);
                    changed = true;
                }
            }
            
            // Also remove from rooms array and clear any tag mapping
        ...
        // For RoomRemoved, also remove from rooms array and clear tag mapping
