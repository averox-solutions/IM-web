...
// import { startDm } from "../../utils/dm/startDm"; // adjust path as needed
...
export default class RoomList extends React.PureComponent<IProps, IState> {
    ...
    public componentDidMount(): void {
        ...
        LegacyCallHandler.instance.on(LegacyCallHandlerEvent.ProtocolSupport, this.updateProtocolSupport);
        this.updateLists();
        RoomListStore.instance.emit(LISTS_UPDATE_EVENT); // trigger the first update
    }
    ...
    private renderSublists(): React.ReactElement[] {
        ...
        const myUserId =
            (this.context && this.context.getUserId && this.context.getUserId()) ||
            (peg ? peg.getUserId() : "") ||
            "";
        ...
        return TAG_ORDER.map((orderedTagId) => {
            ...
            const filteredRooms = allRooms.filter((room) => {
                if (orderedTagId === DefaultTagID.DM) {
                    ...
                    const partner = dmUserId && room.getMember?.(dmUserId);
                    const myMembership = room.getMyMembership?.();
            
                    const isGroupRoom = joined.length > 2;
                    const isBrokenDm = joined.length === 1 || (partner && partner.membership !== "join");
            
                    if (isBrokenDm && myMembership === "join" && !isGroupRoom) {
                        setTimeout(() => {
                            MatrixClientPeg.safeGet().leave(room.roomId)
                                .then(() => console.log(`🚪 Left empty 1:1 DM room: ${room.roomId}`))
                                .catch((err) => console.warn(`❌ Failed to leave empty DM room ${room.roomId}:`, err));
                        }, 100);
                        return false;
                    }
            
                    return !isBrokenDm && !isGroupRoom;
                }
                ...
                if (orderedTagId === DefaultTagID.Untagged) {
                    ...
                    const isDepartmentRoom =
                        ...
                        room.getCanonicalAlias?.()?.includes("ms2.beep.gov.pk");
                
                    return (joined.length > 1 || isDepartmentRoom);
                }
