...
// Build a power-levels content where both 1:1 participants start at 50
function buildDmPowerLevels(creatorId: string, inviteeId?: string) {
    const users: Record<string, number> = { [creatorId]: 50 };
    if (inviteeId) users[inviteeId] = 50;

    return {
        users,
        users_default: 0,
        // Require 50 for changing state; messages remain at 0
        events_default: 0,
        state_default: 50,
        ban: 50,
        kick: 50,
 ...
 * @param {string=} opts.dmUserId If specified, make this a DM room for this user and invite them
 * @param {object=} opts.createOpts set of options to pass to createRoom call.
 * @param {bool=} opts.spinner True to show a modal spinner while the room is created. Default: True
 * @param {bool=} opts.guestAccess Whether to enable guest access. Default: True
 * @param {bool=} opts.encryption Whether to enable encryption. Default: False
 * @param {bool=} opts.inlineErrors True to raise errors off the promise instead of resolving to null. Default: False
 * @param {bool=} opts.andView True to dispatch an action to view the room once it has been created.
 *
 * @returns {Promise} which resolves to the room id, or null if the action was aborted or failed.
 */
export default async function createRoom(client: MatrixClient, opts: IOpts): Promise<string | null> {
    opts = opts || {};
    if (opts.spinner === undefined) opts.spinner = true;
    ...
    // Ensure DMs are marked as direct and use the DM preset
    if (opts.dmUserId) {
        createOpts.is_direct = true;
        createOpts.preset = Preset.TrustedPrivateChat;
    }
    ...
    // Allow guests by default since the room is private and they'd need an invite.
    if (opts.guestAccess) {
        createOpts.initial_state.push({
            type: "m.room.guest_access",
            state_key: "",
    ...
    if (opts.avatar) {
        ...
        createOpts.initial_state.push({ type: EventType.RoomAvatar, content: { url } });
    }
    ...
    // Apply 50/50 power levels ONLY for true 1:1 DMs (exactly one invitee with MXID)
    if (opts.dmUserId && Array.isArray(createOpts.invite) && createOpts.invite.length === 1) {
        const creator = client.getSafeUserId();
        const invitee = createOpts.invite[0];
        createOpts.initial_state.push({
            type: EventType.RoomPowerLevels,
            state_key: "",
            content: buildDmPowerLevels(creator, invitee),
        });
    }
    ...
    return client
        ...
        .then(
            ...
            function (err) {
                ...
                dis.dispatch({ action: Action.JoinRoomError, roomId });
                logger.error("Failed to create room " + roomId + " " + err);
                let description = _t("create_room|generic_error");
                if (err.errcode === "M_UNSUPPORTED_ROOM_VERSION") {
                    description = _t("create_room|unsupported_version");
...
export async function canEncryptToAllUsers(client: MatrixClient, userIds: string[]): Promise<boolean> {
    try {
        const usersDeviceMap = await client.getCrypto()?.getUserDeviceInfo(userIds, true);
        if (!usersDeviceMap) return false;
...
// Similar to ensureDMExists but also adds creation content
// without polluting ensureDMExists with unrelated stuff (also they're never encrypted).
export async function ensureVirtualRoomExists(
    client: MatrixClient,
    userId: string,
    nativeRoomId: string,
...
export async function checkUserIsAllowedToChangeEncryption(
    ...
    // server is forcing encryption to ENABLED while .well-known config is forcing it to DISABLED
    // server version config overrides wk config
    if (doesServerForceEncryptionForPreset && doesWellKnownForceDisableEncryption) {
        console.warn(
            `Conflicting e2ee settings: server config and .well-known configuration disagree. Using server forced encryption setting for chat type ${chatPreset}`,
