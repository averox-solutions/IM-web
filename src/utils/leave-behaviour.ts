...
export async function leaveRoomBehaviour(
    ...
    // This clears the "sticky room" state in RoomListStore, allowing the room to be removed from the list.
    if (SdkContextClass.instance.roomViewStore.getRoomId() === roomId) {
        if (isMetaSpace(SpaceStore.instance.activeSpace)) {
            dis.dispatch<ViewHomePagePayload>({ action: Action.ViewHomePage });
        } else if (SpaceStore.instance.activeSpace === roomId) {
        ...
        // Allow a brief moment for the navigation to propagate to stores
        await sleep(50);
    }

    // 2. UI Update: Explicitly remove the room from the room list immediately.
    // Now that we've navigated away, the room is no longer "sticky" and will be strictly removed.
    try {
        await RoomListStore.instance.manualRoomUpdate(room, RoomUpdateCause.RoomRemoved);
    } catch (e) {
        // Non-critical: the room list will eventually update on the next sync
        console.warn("Failed to immediately remove room from list after leave:", e);
    }
    ...
    // 5. Forget the room to fully clean it up
    try {
        await matrixClient.forget(roomId);
    } catch (e) {
        console.warn("Failed to forget room after leaving:", e);
    }
}
