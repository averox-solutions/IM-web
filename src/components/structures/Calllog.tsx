...
export const Calllog = () => {
    ...
    const decryptValue = (encryptedBase64: string, key: CryptoJS.lib.WordArray, iv: CryptoJS.lib.WordArray): string => {
        try {
            // Parse the base64 string and create a CipherParams object
    ...
    const fetchCallLogs = async () => {
        ...
        try {
            ...
            // Generate AES key & IV from SHA256 hash of rememberKey
            ...
            const apiKey = process.env.REACT_APP_BEEP_API_KEY || "";
            if (!apiKey) {
            ...
            const response = await fetch(`https://bservices-api.org.pk/api/call-logs/${encodeURIComponent(userId)}`, {
                method: "GET",
                headers: {
                    "x-api-key": process.env.REACT_APP_BEEP_API_KEY,
                },
            });
            ...
            const decryptedLogs = encryptedLogs.map((log: any): CallLog => {
                let hasError = false;
            ...
            const sortedLogs = decryptedLogs.sort((a: CallLog, b: CallLog) => 
