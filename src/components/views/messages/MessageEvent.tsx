...
export default class MessageEvent extends React.Component<IProps> implements IMediaBody, IOperableEventTile {
    ...
    public componentDidMount(): void {
        this.props.mxEvent.addListener(MatrixEventEvent.Decrypted, this.onDecrypted);
        this.props.mxEvent.addListener(MatrixEventEvent.Replaced, this.onEventReplaced);
    }

    public componentWillUnmount(): void {
        this.props.mxEvent.removeListener(MatrixEventEvent.Decrypted, this.onDecrypted);
        this.props.mxEvent.removeListener(MatrixEventEvent.Replaced, this.onEventReplaced);
        this.mediaHelper?.destroy();
    }
    ...
    private onEventReplaced = (): void => {
        // Event has been edited → force re-render
        this.forceUpdate();
    };
    ...
    /**
     * Validates that the filename extension and advertised mimetype
     * match for images/videos. For thumbnails, validates mimetype as image.
     */
    private validateImageOrVideoMimetype = (content: IContent): boolean => {
        const filename = content.filename ?? content.body;
        if (!filename) return false;

        if (!this.validateThumbnailMimetype(content)) return false;
    ...
    private validateStickerMimetype = (content: IContent): boolean => {
        if (!this.validateThumbnailMimetype(content)) return false;
        const contentMimetype = content.info?.mimetype;
        return !contentMimetype || this.validateAllowedMimetype(contentMimetype, ["image"]);
    };
